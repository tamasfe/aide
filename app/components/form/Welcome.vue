...
<template>
  <div class="flex flex-col items-center justify-center gap-y-3 text-center">
    <h1 class="text-2xl md:text-3xl font-semibold text-white">{{ $t("welcome_form.title", { siteName: siteStore.site.name }) }}</h1>
    ...
    <p class="text-subtle">{{ $t("welcome_form.description") }}</p>
  </div>
</template>
