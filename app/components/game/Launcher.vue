...
<template>
  <div>
    <template v-if="session">
      ...
      <div
        v-else
        class="w-full h-full px-6 flex flex-col items-center justify-center bg-subtle"
      >
        <NuxtImg
          class="h-7"
          ...
          alt="Logo"
        />
        <template v-if="session.errorName === 'ErrorInsufficientFunds'">
          <h2 class="mt-6 text-2xl font-semibold">
            {{ $t("game.game_launcher.error_insufficient_funds_title") }}
          </h2>
          <p class="text-center">
            {{ $t("game.game_launcher.error_insufficient_funds_message") }}
          </p>
        ...
        <template v-if="session.errorName === 'ErrorWalletNotFound'">
          <h2 class="mt-6 text-2xl font-semibold">
            {{ $t("game.game_launcher.error_wallet_not_found_title") }}
          </h2>
          <p class="text-center">
            {{ $t("game.game_launcher.error_wallet_not_found_message") }}
          </p>
          <div class="mt-6 flex items-center gap-4">
            <BaseButton
              id="app-header-deposit-button"
        ...
        <template v-else-if="session.errorName === 'ErrorGameNotAccessible'">
          <h2 class="mt-6 text-2xl font-semibold">
            {{ $t("game.game_launcher.error_game_not_accessible_title") }}
          </h2>
          <p class="text-center">
        ...
        <template v-else-if="session.errorName === 'ErrorGameHasNoDemo'">
          <h2 class="mt-6 text-2xl font-semibold">
            {{ $t("game.game_launcher.error_game_has_no_demo_title") }}
          </h2>
          <p class="text-center">
            {{ $t("game.game_launcher.error_game_has_no_demo_message") }}
          </p>
          <div class="mt-6 flex items-center gap-4">
            <BaseButton
              id="app-header-deposit-button"
        ...
        <template v-else-if="session.errorName === 'ErrorUnauthorized'">
          <h2 class="mt-6 text-2xl font-semibold">
            {{ $t("game.game_launcher.error_unauthorized_title") }}
          </h2>
          <p class="text-center">
            {{ $t("game.game_launcher.error_unauthorized_message") }}
          </p>
          <div class="mt-6 flex items-center gap-4">
            <BaseButton
              id="app-header-deposit-button"
        ...
        <template v-else>
          <h2 class="mt-6 text-2xl font-semibold">
            {{ $t("game.game_launcher.error_generic_title") }}
          </h2>
          <p class="text-center">
