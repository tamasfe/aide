...
const { data: pageAml } = useAsyncData("aml-policy-" + locale.value, async () => {
  ...
  if (!content) {
    logger.error(`!! AML Policy content not found, this is critical as the user may see a blank page for a required legal document`, new ErrorTranslatedContentNotFound(locale.value, "aml_policy"));
  }
  return content;
}, {
...
const { data: pageKyc } = useAsyncData("kyc-policy-" + locale.value, async () => {
  ...
  if (!content) {
    logger.error(`!! KYC Policy content not found, this is critical as the user may see a blank page for a required legal document`, new ErrorTranslatedContentNotFound(locale.value, "kyc_policy"));
  }
  return content;
}, {
  watch: [() => locale.value], // Refetch when locale changes
