...
const { data } = useAsyncData("history-page-deposits-data", async () => {
  ...
  loading.value = true;
  const data = await wallet.ui.searchPaymentsOnTable.handle(walletStore.wallet.wallet_id, "deposit", pageIndex.value);
...
<template>
  ...
  >
    <template #empty>
      <BaseEmpty
        :title="$t('account.history.deposits.table_empty')"
