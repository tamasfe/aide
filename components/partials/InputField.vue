<template>
  <div class="relative w-full">
    <input
      ...
      placeholder=" "
      :value="value"
      @input="type === 'phone' ? emit('update:modelValue', countryModel + model) : emit('update:modelValue', model)"
    >
    <label
      for="input"
    ...
    <div v-if="type==='phone'" class="w-[93px] h-full absolute right-0 top-0">
      <div class="absolute top-[30px] -translate-y-1/2 left-2">
        <span :class="'fi fi-' + selectedCountryCode.toLocaleLowerCase()"></span>
      </div>
      ...
      >
      <Transition>
        <div v-if="showSelect" class="select__container absolute w-full h-[140px] bottom-full bg-[#2F323C] border border-[#3d404d] rounded shadow-2xl overflow-hidden overflow-y-auto">
          <div
          ...
          >
            <span class="mr-1 w-5">
              <span :class="'fi fi-'+ country.code.toLocaleLowerCase()"></span>
            </span>
...
const selectCountry = (country:any) => {
  countryModel.value = country.dial_code
}
...
const filteredCountries = computed(() => {
  const find = countryCodes.filter(el => el.dial_code.includes(countryModel.value))
