...
*/}}
{{- define "frontend.priorityClassName" -}}
{{- if .Values.priorityClass.name }}
{{- .Values.priorityClass.name }}
{{- else }}
{{- printf "%s-high-priority" (include "frontend.fullname" .) }}
{{- end }}
{{- end }}
...
{{- if or .Values.affinity.podAffinity.enabled .Values.affinity.podAntiAffinity.enabled }}
affinity:
    ...
    preferredDuringSchedulingIgnoredDuringExecution:
      - weight: {{ .Values.affinity.podAffinity.weight }}
        podAffinityTerm:
          ...
          labelSelector:
            matchLabels:
              {{- range $key, $value := .Values.affinity.podAffinity.labelSelector }}
              {{ $key }}: {{ $value }}
              {{- end }}
  {{- end }}
    ...
    preferredDuringSchedulingIgnoredDuringExecution:
      - weight: {{ .Values.affinity.podAntiAffinity.weight }}
        podAffinityTerm:
          ...
          labelSelector:
            matchLabels:
              {{- include "frontend.selectorLabels" . | nindent 14 }}
  {{- end }}
{{- end }}
{{- end }}